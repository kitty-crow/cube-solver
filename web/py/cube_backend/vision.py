...
class TileBank:
    """Compact, rotation-aware border descriptors for all 54 captured stickers."""

    def __init__(self, raw: bytes, size: int):
        expected = 54 * size * size * 3
        if len(raw) != expected:
    ...
    def _make_profile(self, tile: int, rot: int, side: str) -> tuple[tuple[float, float, float, float], ...]:
        ...
        for i in range(samples):
            ...
            for d in range(depth0, depth0 + depth_count):
                if side == "N": x, y = int(t), d
                elif side == "S": x, y = int(t), n - 1 - d
                elif side == "W": x, y = d, int(t)
                else: x, y = n - 1 - d, int(t)
    ...
    def compatibility(self, a: int, ar: int, aside: str, b: int, br: int, bside: str) -> float:
        ...
        for x, y in zip(pa, pb):
            # Chromaticity survives exposure changes well. Centred luminance keeps map texture/coast detail.
