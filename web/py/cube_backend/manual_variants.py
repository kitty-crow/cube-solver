...
def _normalise_locked_faces(values, requested: list[int]) -> set[int]:
    ...
    for value in values or []:
        ...
        if 0 <= face < 6:
            locked.add(face)
    # Backward-compatible callers did not send an explicit lock list. A non-zero
    # correction supplied by the user must still be treated as authoritative.
    if not locked:
...
def _choose_reachable_centre_corrections(
    ...
    for candidate_tuple in product(range(4), repeat=6):
        ...
        # The picture-centre subgroup generated by the legal centre algorithms
        # has even total quarter-turn parity. This is the same reachability test
        # used by centre_fit before the exact shortest correction is requested.
