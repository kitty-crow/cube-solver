        ...
        for (mask, flip, parity), (score, path) in dp.items():
            for q in range(12):
                ...
                for cand in scored[p][q]:
                    ...
                    old = nxt.get(key)
                    if old is None or value[0] > old[0]: nxt[key] = value
        dp = nxt
    full = (1 << 12) - 1
    return {parity: value for parity in (0, 1) if (value := dp.get((full, 0, parity))) is not None}
        ...
        for k in range(4):
            ...
            for cand in touching:
                for p in cand.placements:
                    if p.target_face != face: continue
                    r, c = _rc(_local_of_facelet(p.target_facelet))
                    side = _side_between((r, c), (1, 1))
...
def _facelet_string(edges: tuple[EdgeCandidate, ...], corners: tuple[CornerCandidate, ...]) -> str:
    out = ["?"] * 54
    for f in range(6): out[CENTER_FACELET[f]] = FACE_NAMES[f]
    for cand in edges:
        for current_slot, p in enumerate(cand.placements): out[EDGE_FACELET[cand.current_pos][current_slot]] = FACE_NAMES[p.target_face]
    for cand in corners:
        for current_slot, p in enumerate(cand.placements): out[CORNER_FACELET[cand.current_pos][current_slot]] = FACE_NAMES[p.target_face]
    if "?" in out: raise ValueError("Internal reconstruction error: incomplete facelet string")
    return "".join(out)
